...
void Hyphenate::HyphenationTree::insert(auto_ptr<HyphenationRule> pattern) {
   ...
   std::wstring gucs = cefix::Unicode::UTF8toWstring(pattern->getKey());
    std::string gs = cefix::Unicode::wstringToUTF8(cefix::strToLower(gucs));
   
   root->insert(gs.c_str(), pattern);
   ...
   (const string &word, size_t stop_at) const
{
   ...
   std::string w = ".";
   {
      ...
      std::wstring gucs = cefix::Unicode::UTF8toWstring(word);
      std::string gs = cefix::Unicode::wstringToUTF8(cefix::strToLower(gucs));
