   ...
   for (unsigned int i = 0; i < word.size(); i++) {
      ...
      if ( (word[i] & 0xC0) == 0x80)
         ;
          ...
          unsigned int ch = cefix::Unicode::getCodePointFromUTF8(word.c_str()+i);
          bool isalpha = std::iswalpha(ch);
         ...
         throw(std::domain_error)
{
    ...
    for (unsigned int i = 0; i < len; i++)
        cur = cefix::Unicode::jumpToNextUTF8CodePoint(cur);
   
    const char *next = cur;
    if (!isspace(cefix::Unicode::getCodePointFromUTF8(next)))
        next = cefix::Unicode::jumpToNextUTF8CodePoint(next);
   ...
   if ( isspace(cefix::Unicode::getCodePointFromUTF8(next)) ) {
      ...
      while ( isspace(cefix::Unicode::getCodePointFromUTF8(cur)) )
          cur = cefix::Unicode::jumpToPrevtUTF8CodePoint(cur);
      int len = cur - src.c_str() + 1;
      result.first = src.substr(0, len);

      /* Neither do we leave spaces at the beginning of the next. */
      while ( isspace(cefix::Unicode::getCodePointFromUTF8(next)) )
         next = cefix::Unicode::jumpToNextUTF8CodePoint(next);
      ...
      while (true) {
         ...
         bool in_word = iswalpha(cefix::Unicode::getCodePointFromUTF8(cur));
         const char *word_start = NULL;
         while ( cur > src.c_str() ) {
            cur = cefix::Unicode::jumpToPrevtUTF8CodePoint(cur);
            gunichar ch = cefix::Unicode::getCodePointFromUTF8(cur);

            if (in_word && (!iswalpha(ch))) {
               /* If we have a word, try hyphenating it.*/
               word_start = cefix::Unicode::jumpToNextUTF8CodePoint(cur);
               break;
            } else if (isspace(ch)) {
               break;
            } else if (!in_word && iswalpha(ch))
               in_word = true;
         ...
         if (word_start != NULL) {
            ...
            * the end. */
            ...
            while ( iswalpha(cefix::Unicode::getCodePointFromUTF8(word_end)) )
               word_end = cefix::Unicode::jumpToNextUTF8CodePoint(word_end);
            ...
            * fit. */
            ...
            for (const char *i = src.c_str(); i <= word_start; 
                 i = cefix::Unicode::jumpToNextUTF8CodePoint(i))
               if (isspace(cefix::Unicode::getCodePointFromUTF8(i))) {
                  have_space = true;
                  break;
               }
         ...
         if (cur == src.c_str()) {
             ...
             * and move to its end. */
            const char *eol = cur;
            while (*eol != 0 && !isspace(cefix::Unicode::getCodePointFromUTF8(eol)))
               eol = cefix::Unicode::jumpToNextUTF8CodePoint(eol);
            ...
            break;
         } else if (isspace(cefix::Unicode::getCodePointFromUTF8(cur))) {
            /* eol is the end of the previous line, bol the start of the
               * next. */
            const char *eol = cur, *bol = cur;
            while(isspace(cefix::Unicode::getCodePointFromUTF8(eol)))
               eol = cefix::Unicode::jumpToPrevtUTF8CodePoint(eol);
            while(isspace(cefix::Unicode::getCodePointFromUTF8(bol)))
               bol = cefix::Unicode::jumpToNextUTF8CodePoint(bol);
